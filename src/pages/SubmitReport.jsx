    ...
    return (
        ...
        <div className="max-w-xl mx-auto py-6 px-4">
            ...
            <div className="mb-6 text-center">
                <h2 className="text-2xl font-bold font-serif text-slate-900 dark:text-dark-text">{t('report.title')}</h2>
            ...
            <div className="bg-white dark:bg-dark-surface rounded-xl shadow-lg border border-slate-200 dark:border-dark-border overflow-hidden relative">
                {step === 1 && (
                    <div className="p-6 animate-in slide-in-from-right duration-300">
                        <h3 className="text-lg font-bold text-slate-800 dark:text-dark-text mb-4 flex items-center">
                            ...
                            {Object.values(INCIDENT_TYPES).map((type) => {
                                ...
                                return (
                                    <button
                                        ...
                                        className="flex flex-col items-center justify-center p-6 border-2 border-slate-100 dark:border-dark-border rounded-xl hover:bg-red-50 dark:hover:bg-red-900/20 hover:border-red-200 dark:hover:border-red-800 hover:shadow-md transition-all gap-3 text-center h-40 group"
                                    >
                                        <div className={cn("p-4 rounded-full text-white transition-transform group-hover:scale-110", type.color)}>
                                            <Icon className="w-8 h-8" />
                                        </div>
                                        <span className="font-bold text-slate-700 dark:text-dark-text group-hover:text-red-700 dark:group-hover:text-red-400">{t(type.translationKey)}</span>
                                    </button>
                                );
                            })}
                        </div>
                ...
                {step === 2 && (
                    <form onSubmit={handleSubmit} className="p-6 animate-in slide-in-from-right duration-300 space-y-6">
                        ...
                        {/* Selected Type Badge */}
                        <div className="flex items-center justify-between bg-slate-50 dark:bg-slate-800 p-3 rounded-lg border border-slate-200 dark:border-dark-border">
                                ...
                                {SEVERITY_OPTIONS.map((level) => (
                                    <div 
                                        ...
                                        className={cn(
                                            ...
                                            formData.severity === level.id 
                                                ? "border-slate-800 dark:border-slate-600 bg-slate-50 dark:bg-slate-800 ring-1 ring-slate-800 dark:ring-slate-600" 
                                    ...
                                    >
                                        <div className={cn("w-4 h-4 rounded-full border-2 mr-3 flex items-center justify-center shrink-0", 
                                            formData.severity === level.id ? "border-slate-900 dark:border-slate-300" : "border-slate-300 dark:border-slate-600"
                                        )}>
                                            {formData.severity === level.id && <div className="w-2 h-2 rounded-full bg-slate-900 dark:bg-slate-300"></div>}
                                        </div>
                                        <div className="flex-1">
                                            ...
                                            <p className="text-xs text-slate-500 dark:text-dark-text-muted mt-0.5">{level.desc}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
